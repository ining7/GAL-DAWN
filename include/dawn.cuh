...
public:
    ...
    float sssp_gpu(DAWN::Matrix &matrix, int source, cudaStream_t streams, int *d_A_entry, int *d_A, std::string &output_path);
};
...
void DAWN::runApspGpu(DAWN::Matrix &matrix, std::string &output_path)
{
    ...
    for (int i = 0; i < matrix.rows; i++)
    {
        ...
        elapsed_time += sssp_gpu(matrix, source, streams[cuda_stream], d_A_entry, d_A, output_path);
        proEntry++;
        dawn.infoprint(proEntry, matrix.rows, matrix.interval, matrix.thread, elapsed_time);
    }
    std::cout << ">>>>>>>>>>>>>>>>>>>>>>>>>>> APSP end <<<<<<<<<<<<<<<<<<<<<<<<<<<" << std::endl;
...
void DAWN::runSsspGpu(DAWN::Matrix &matrix, std::string &output_path)
{
    ...
    float elapsed_time = sssp_gpu(matrix, source, stream, d_A_entry, d_A, output_path);
...
float DAWN::sssp_gpu(DAWN::Matrix &matrix, int source, cudaStream_t streams, int *d_A_entry, int *d_A, std::string &output_path)
{
