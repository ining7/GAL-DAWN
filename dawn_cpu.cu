...
public:
	Matrix() {}
	Matrix(string _g_type, string _device, AlgoType _algo_type, int _row, int _col) : g_type(_g_type), device(_device), algo_type(_algo_type), row(_row), col(_col)
	{
...
#pragma omp parallel for
	...
	{ // this = this * A
		...
		for (int i = 0; i < this->row; ++i)
		{
			...
			if (i % (this->row / 1000) == 0)
			{
				float completion_percentage =
					static_cast<float>(i * 100.0f) / static_cast<float>(this->row);
...
{
public:
	Graph(string _g_type, string _device, int _node_num, AlgoType _algo_type) : node_num(_node_num), algo_type(_algo_type)
	{
		k = 0;
...
#pragma omp parallel for
	...
	void runDawn(string g_type, string device)
	{
		...
		while (1)
		{
			...
			{
				// cout << "denseCpuMultiplication" << endl;
			...
			std::cout << "执行需要计时的操作所用时间为：" << time_operation << " 秒。"
					  << std::endl;

			// cout << "updateShortestPath" << endl;
		...
		std::cout << "GPU Elapsed Time:" << time_operation << " s."
				  << std::endl;
	}
	void runDij() {}
	void runSpfa() {}
