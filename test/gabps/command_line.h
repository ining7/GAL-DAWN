...
class CLBase {
 protected:
  ...
  char** argv_;
  std::string name_;
  std::string get_args_ = "f:z:g:hk:su:m";
  std::vector<std::string> help_strings_;

  int scale_ = -1;
  int degree_ = 16;
  std::string filename_ = "";
  std::string input_node_list_address_ = "";
  bool symmetrize_ = false;
  bool uniform_ = false;
  bool in_place_ = false;

  void AddHelpLine(char opt,
                   std::string opt_arg,
                   std::string text,
                   std::string def = "") {
 ...
 public:
  CLBase(int argc, char** argv, std::string name = "")
    ...
    AddHelpLine('z', "file", "address of input node list");
  ...
  void virtual HandleArg(signed char opt, char* opt_arg) {
    switch (opt) {
      case 'f':
        ...
        break;
      case 'z':
        input_node_list_address_ = std::string(opt_arg);
        break;
      case 'g':
        scale_ = atoi(opt_arg);
        break;
  ...
  std::string input_node_list_address() const {
    return input_node_list_address_;
  }
