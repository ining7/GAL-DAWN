...
void writeList(Matrix& len, Matrix& A, Matrix& B, int select_direction);
void DAWN(Matrix& len, Matrix& A, Matrix& B, int k, int k_max, int k_diameter);
void unweightedPipeline(Matrix& len, int nodes, int select_direction);
...
void printMatrix(Matrix A) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            cout << setw(stw) << getElement(A, i, j) << ' ';
        }
        cout << '\n';
    }
}
...
void writeList(Matrix& len, Matrix& A, Matrix& B, int select_direction) {
    ...
    while(infile) {
        ...
        setElement(A, a, b, 1);
        setElement(B, a, b, 1);
        setElement(len, a, b, 1);
        if (select_direction == 1) {
            setElement(A, b, a, 1);
            setElement(B, b, a, 1);
            setElement(len, b, a, 1);
        }
    }
    infile.close();
}
...
void unweightedPipeline(Matrix& len, int nodes, int select_direction) {
    ...
    writeList(len, A, B, select_direction);
    ...
    DAWN(len, A, B, k, k_max, k_diameter);
...
int main(int argc, char *argv[]) {
    ...
    unweightedPipeline(shortest_distance, nodes, select_direction);

    // Output the shortest path result
    printMatrix(shortest_distance);
